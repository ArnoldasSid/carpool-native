...
import { requestRide, withdrawRideRequest, completeTrip, acceptRideRequest } from '../actions.js';
import { YOUR_ROLE_UPDATED, USERS_RIDE_REQUEST_GOT_ACCEPTED } from '../constants.js';
  ...
  describe('idle flow', () => {
    it('Updates role to rider after ride request', () => {
      ...
      expect(
        store
          .getActions()
          .some(action => action.type === YOUR_ROLE_UPDATED && action.payload.newRole === 'REQUESTER'),
      ).toBe(true);
    });
  });
  describe('requester flow', () => {
    ...
    it('Sets role to none after user withdraws ride request', () => {
      ...
      expect(
        store
          .getActions()
          .some(action => action.type === YOUR_ROLE_UPDATED && action.payload.newRole === 'NONE'),
      ).toBe(true);
    });
    ...
    it('Sets role to rider if request gets accepted', () => {
      ...
      expect(
        store
          .getActions()
          .some(action => action.type === YOUR_ROLE_UPDATED && action.payload.newRole === 'RIDER'),
      ).toBe(true);
    });
    ...
    it('Sets role to driver if user accepts ride request', () => {
      ...
      expect(
        store
          .getActions()
          .some(action => action.type === YOUR_ROLE_UPDATED && action.payload.newRole === 'DRIVER'),
      ).toBe(true);
    });
  });
  describe('rider flow', () => {
